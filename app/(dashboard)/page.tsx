...
export default async function DashboardHomePage() {
    ...
    const { count: bkkCount } = await supabase.from('informants').select('*', { count: 'exact', head: true }).eq('canal_zone', 'บางเขน')
    const { count: premCount } = await supabase.from('informants').select('*', { count: 'exact', head: true }).eq('canal_zone', 'เปรมประชากร')
    const { count: latCount } = await supabase.from('informants').select('*', { count: 'exact', head: true }).eq('canal_zone', 'ลาดพร้าว')
